...
from .controller import (create_user, get_all_users, get_user_by_id, 
                         update_user_status, authenticate_user, delete_user_session, 
                         require_session, get_local_user
)
...
@router.get('/local/')
def user_get_local(db: Session = Depends(get_db),
                   _: None = Depends(require_session)
                   ):
...
def user_login(login_data: LoginData,
    ...
    auth = authenticate_user(db, login_data)
    return auth
...
def user_logout(request: Request,
    ...
    try:
        delete_user_session(request, db)
    except:
